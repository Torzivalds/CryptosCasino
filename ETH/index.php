...
if (!empty($_POST['paymentID']) AND !empty($_POST['publicAdress'])) {
	...
	$req = $bdd->query("SELECT `Numero` FROM `ETH` WHERE 1 ORDER BY ID DESC LIMIT 1");
	$numeroPartieNow = 1;
	...
	$req = $bdd->prepare("INSERT INTO `ETH`(`WalletAdress`, `IDTransaction`, `Resultat`, `Numero`, `DateJeu`) VALUES (?, ?, ?, ?, ?)");
							...
							<article class="post featured">
								<header class="major">
									...
									<p>
										...
										Each player sends 0.1 ETH and tries to win 1 ETH!<br>
										It's not a question of luck, it's the 25th player who wins, then it starts from scratch, and starts again.</p>
								</header>
								<h2>You too, come play to try to win one Ethereum (1 ETH) !!</h2><br>
								<form method="post" action="index.php">
									<div id="deuxChamps">
										<p>
											Send 0.1 ETH to this adress :
									...
									<input type="text" name="publicAdress" placeholder="Enter your public Ethereum adress" required>
									If you win, you will receive a Ethereum on this wallet.<br>
									<h6 style="font-style: italic;">If you send the form but the transaction you need to make is not done,<br>your Ethereum address may be banned from the site.</h6>
