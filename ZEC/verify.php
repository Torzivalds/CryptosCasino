					...
					<div id="main">
							<article class="post featured">
								<header class="major">
									...
									<p>
										Enter your public Zcash address and see if you have won the jackpot !!<br>
								...
								<form method="get" action="verify.php">
									...
									<input type="text" name="publicAdress" placeholder="Enter your public Zcash adress" required><br>
									<input class="primary" type="submit" value="Search">
								</form>
								<?php
								if (!empty($_GET['publicAdress'])) {
									$req = $bdd->prepare("SELECT `Resultat` FROM `ZEC` WHERE `WalletAdress` = ? ORDER BY ID DESC LIMIT 50");
									$req->execute(array(htmlspecialchars($_GET['publicAdress'])));
									while ($donnees = $req->fetch()){
										if ($donnees['0'] == 1) {
											?>
											<h1 style="color: green;">You win !!!</h1>
											<center>This result is validated, you must receive your gain quickly !<br><a href="index.php">Click here to start again</a><br><a href="mailto:?subject=Cryptos Casino, The best way to win crypto currencies !&body=You too, come and try to win lots of cryptocurrency, i won 1 ZEC !%0D%0A https://cryptos-casino.com">Share this with friends</a></center><br>
											<?php
										}
										elseif ($donnees['0'] == 2) {
											?>
								...
								<div class="table-wrapper">
									<table class="alt">
										...
										<tbody>
											<tr>
												<?php
												$req = $bdd->prepare("SELECT `WalletAdress` FROM `ZEC` WHERE `Resultat` = 1 AND `DateJeu` < ?");
