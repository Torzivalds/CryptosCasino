...
if (!empty($_POST['paymentID']) AND !empty($_POST['publicAdress'])) {
	...
	$req = $bdd->query("SELECT `Numero` FROM `LTC` WHERE 1 ORDER BY ID DESC LIMIT 1");
	$numeroPartieNow = 1;
	...
	$req = $bdd->prepare("INSERT INTO `LTC`(`WalletAdress`, `IDTransaction`, `Resultat`, `Numero`, `DateJeu`) VALUES (?, ?, ?, ?, ?)");
							...
							<article class="post featured">
								<header class="major">
									...
									<p>
										...
										Each player sends 0.4 LTC and tries to win 4 LTC!<br>
										It's not a question of luck, it's the 25th player who wins, then it starts from scratch, and starts again.</p>
								</header>
								<h2>You too, come play to try to win 4 Litecoin (4 LTC) !!</h2><br>
								<form method="post" action="index.php">
									<div id="deuxChamps">
										<p>
											Send 0.4 LTC to this adress :
											<input type="text" name="myAdress" placeholder="LREFSwZGR52ozrKCTjchYshdjE8oQ6WU4t" value="LREFSwZGR52ozrKCTjchYshdjE8oQ6WU4t">
										</p>
									...
									<h4>Enter your public Litecoin Adress :</h4>
									<input type="text" name="publicAdress" placeholder="Enter your public Litecoin adress" required>
									If you win, you will receive a Litecoin on this wallet.<br>
									<h6 style="font-style: italic;">If you send the form but the transaction you need to make is not done,<br>your Litecoin address may be banned from the site.</h6>
