...
if (!empty($_POST['paymentID']) AND !empty($_POST['publicAdress'])) {
	...
	$req = $bdd->query("SELECT `Numero` FROM `BTC` WHERE 1 ORDER BY ID DESC LIMIT 1");
	$numeroPartieNow = 1;
	...
	$req = $bdd->prepare("INSERT INTO `BTC`(`WalletAdress`, `IDTransaction`, `Resultat`, `Numero`, `DateJeu`) VALUES (?, ?, ?, ?, ?)");
							...
							<article class="post featured">
								<header class="major">
									...
									<p>
										...
										Each player sends 0.003 BTC and tries to win 0.03 BTC!<br>
										It's not a question of luck, it's the 25th player who wins, then it starts from scratch, and starts again.</p>
								</header>
								<h2>You too, come play to try to win 0.03 Bitcoin (0.03 BTC) !!</h2><br>
								<form method="post" action="index.php">
									<div id="deuxChamps">
										<p>
											Send 0.003 BTC to this adress :
											<input type="text" name="myAdress" placeholder="13hrSopEXEcTxbxccwReM6WtSvEDBNH6RU" value="13hrSopEXEcTxbxccwReM6WtSvEDBNH6RU">
										</p>
									...
									<h4>Enter your public Bitcoin Adress :</h4>
									<input type="text" name="publicAdress" placeholder="Enter your public Bitcoin adress" required>
									If you win, you will receive a Bitcoin on this wallet.<br>
									<h6 style="font-style: italic;">If you send the form but the transaction you need to make is not done,<br>your Bitcoin address may be banned from the site.</h6>
